...
import { MetroparkRedirectionData, MetroparkRedirectionIntegration } from '@integrations/crawlers/metropark/metropark-redirection.integration';
...
@Injectable()
export class MetroparkRedirectionService {
    constructor(
        ...
        private metroparkRedirectionIntegration: MetroparkRedirectionIntegration,
    ) {}
    ...
    async redirect(
        ...
        if (!ownerAddressDetails.valid) {
            socket.emit('municipal-redirection', { message: ownerAddressDetails.message, type: 'error' });
            throw new InternalServerErrorException({
                message: ownerAddressDetails.message,
                detail: { ownerAddressDetails },
            });
        }
        ...
        //////////////////////////////////////////////////////////////////
        // Setup the data
        const data: MetroparkRedirectionData = {
            ...
            customerPostOffice: userAddressDetails?.streetNumber,
            ...
            externalCode: infringement.issuer.integrationDetails.code,
        };
        ...
        if (!!driverContract?.contractId) {
            ...
            data.customerPostOffice = driverAddressDetails?.streetNumber;
            data.custDrivingLicense = driverContract.driver.licenseNumber;
            data.driver = JSON.stringify(driverContract.driver);
        }

        const redirectionData = plainToClass(MetroparkRedirectionData, data);

        this.logger.debug({
            message: 'Redirection data for Metropark',
        ...
        if (validationErrors.length > 0) {
            ...
            throw new InternalServerErrorException({
                message: 'Invalid redirection data, please contact support',
                detail: validationErrors,
            });
        }
        ...
        try {
            const response = await this.metroparkRedirectionIntegration.redirectInfringement(redirectionData);
            ...
            this.logger.debug({ message: 'Metropark Response', detail: response, fn: this.redirect.name });
        } catch (e) {
            ...
            this.logger.error({
                message: 'Metropark Request failed',
                detail: {
                    error: e.message,
                    stack: e.stack,
                },
