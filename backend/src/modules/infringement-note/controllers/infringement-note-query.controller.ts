...
@Controller('query/infringement-note')
export class InfringementNoteQueryController extends QueryController {
    alias = 'infringementNote';
    ...
    ): Promise<PaginationResponseInterface<InfringementNote>> {
        // FIXME: insert base query
        const baseQuery = InfringementNote.findWithMinimalRelations();
        return await PaginatedFilterQuery.paginatedFilterQuery<InfringementNote>(req, baseQuery, 'infringementNote');
    }
    ...
    async getManyAsSpreadsheet(
        ...
        res.setHeader('Content-Disposition', `attachment; filename=filtered-infringement-notes-${moment().format('DD-MM-YYYY')}.xlsx`);
