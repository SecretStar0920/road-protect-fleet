...
import { ManualInfringementRedirectionSpreadsheetDto } from '@modules/infringement/controllers/manual-infringement-redirection-spreadsheet.dto';
...
@Injectable()
export class ManualInfringementRedirectionSpreadsheetService extends SpreadsheetService<ManualInfringementRedirectionSpreadsheetDto> {
    constructor(
        protected logger: Logger,
        private manualInfringementRedirectionService: ManualInfringementRedirectionService,
    ...
    async checkValidity(spreadsheetData: any[]): Promise<UploadSpreadsheetResult<ManualInfringementRedirectionSpreadsheetDto>> {
        const results: UploadSpreadsheetResult<ManualInfringementRedirectionSpreadsheetDto> = new UploadSpreadsheetResult<
            ManualInfringementRedirectionSpreadsheetDto
        >();
    ...
    async checkDtoValid(data: any[], results: UploadSpreadsheetResult<ManualInfringementRedirectionSpreadsheetDto>): Promise<void> {
        this.logger.debug({ message: 'Checking Dto valid', detail: undefined, fn: this.checkDtoValid.name });
        // Check row by row
        for (const row of data) {
            const item = plainToClass(ManualInfringementRedirectionSpreadsheetDto, row);
    ...
    async checkDatabaseValid(results: UploadSpreadsheetResult<ManualInfringementRedirectionSpreadsheetDto>): Promise<void> {
    ...
    async upload(dto: SpreadsheetUploadDto, file: MulterFile, socket: DistributedWebsocket) {
        ...
        const jobs = from(verifyResults.valid).pipe(
            ...
            mergeMap((item) => {
                return from(this.manualInfringementRedirectionService.manuallyRedirectInfringement(item)).pipe(
