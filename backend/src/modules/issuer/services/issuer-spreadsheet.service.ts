...
@Injectable()
export class IssuerSpreadsheetService extends SpreadsheetService<CreateIssuerDto> {
    constructor(
    ...
    async checkValidity(spreadsheetData: any[]): Promise<UploadSpreadsheetResult<CreateIssuerDto>> {
        const results: UploadSpreadsheetResult<CreateIssuerDto> = new UploadSpreadsheetResult<CreateIssuerDto>();
        await this.checkDtoValid(spreadsheetData, results);
        await this.checkDatabaseValid(results);
        return results;
    }

    async checkDtoValid(data: any[], results: UploadSpreadsheetResult<CreateIssuerDto>): Promise<void> {
        this.logger.debug({ message: 'Checking Dto valid', detail: undefined, fn: this.checkDtoValid.name });
        // Check row by row
        for (const row of data) {
            const item = plainToClass(CreateIssuerDto, row);
    ...
    async checkDatabaseValid(results: UploadSpreadsheetResult<CreateIssuerDto>): Promise<void> {
        ...
        if (!isEmpty(results.valid)) {
            const query = await Issuer.createQueryBuilder('issuer')
                .select(['issuer.name'])
                .orWhere('issuer.name IN (:...names)', { names: results.valid.map((unique) => unique.name) });
            ...
            remove(results.valid, (item) => {
                ...
                if (foundNames[item.name]) {
                    errors.push({
                        property: 'name',
                        constraints: { unique: `An issuer with this name already exists` },
                        children: [],
                    });
                }
                if (!isEmpty(errors)) {
    ...
    async upload(dto: SpreadsheetUploadDto, file: MulterFile, socket: DistributedWebsocket) {
        ...
        this.logger.log({ message: 'Uploading valid issuers', detail: verifyResults.valid.length, fn: this.upload.name });
        ...
        const jobs = from(verifyResults.valid).pipe(
            ...
            mergeMap((item) => {
                return from(this.createIssuerService.createIssuer(item)).pipe(
                    // If the job fails push to invalid verifyResults
                    catchError((e) => {
                        this.logger.warn({
                            message: 'Failed to upload issuer',
                            detail: {
                                error: e.message,
                                stack: e.stack,
                            },
        ...
        this.logger.log({ message: 'Uploaded issuers', detail: results.valid.length, fn: this.upload.name });
    ...
    async generateValidDocument(data: any): Promise<number> {
        return super.generateValidDocument(data, 'valid-issuers', 'issuer-uploads');
    }

    async generateInvalidDocument(data: any): Promise<number> {
        return super.generateInvalidDocument(data, 'invalid-issuers', 'issuer-uploads');
    }
}
