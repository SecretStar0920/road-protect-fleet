import { Account, Infringement, InfringementNote, Issuer, RawInfringement, RawLocationParserHelper, RedirectionType } from '@entities';
...
/**
 * This is based on the map status function in the telaviv crawler-api code
 */
const telavivFineStatusMapper = {
    1: 'פתוח',
    2: 'שולם',
    7: 'סגור',
};

export class TelavivRawInfringementMapper extends RawInfringementMapper {
    ...
    async getCreateInfringementDto(raw: RawInfringement): Promise<CreateInfringementDto> {
        ...
        const createInfringementData: CreateInfringementDto = {
            ...
            issuer: issuer.code,
            issuerStatus: telavivFineStatusMapper[fine.fine_status],
    ...
    private async getAccount(fine: TelavivRawInfringementDto): Promise<Account> {
        if (fine.fine_end_cust_id) {
            // Either customer identifier is set or the customer name must be used
            const accountIdentifier = fine.fine_end_cust_id;
    ...
    private async getRawInfringementDto(raw: RawInfringement): Promise<TelavivRawInfringementDto> {
        const fine: TelavivRawInfringementDto = plainToClass(TelavivRawInfringementDto, raw.data);
    ...
    async getNominationDtoFromRawInfringement(raw: RawInfringement): Promise<NominationDto> {
        const fine: TelavivRawInfringementDto = await this.getRawInfringementDto(raw);
        if (!fine) {
            return {};
        }
