...
import { KfarSabaRedirectionData, KfarSabaRedirectionIntegration } from '@integrations/crawlers/kfarSaba/kfarSaba-redirection.integration';
...
@Injectable()
export class KfarSabaRedirectionService {
    constructor(
        ...
        private kfarSabaRedirectionIntegration: KfarSabaRedirectionIntegration,
    ) {}
    ...
    async redirect(
        ...
        //////////////////////////////////////////////////////////////////
        // Setup the data
        const data: KfarSabaRedirectionData = {
        ...
        const redirectionData = plainToClass(KfarSabaRedirectionData, data);

        this.logger.debug({
            message: 'Redirection data for KfarSaba',
        ...
        try {
            const response = await this.kfarSabaRedirectionIntegration.redirectInfringement(redirectionData);
            ...
            this.logger.debug({ message: 'KfarSaba Response', detail: response, fn: this.redirect.name });
        } catch (e) {
            ...
            this.logger.error({
                message: 'KfarSaba Request failed',
                detail: {
                    error: e.message,
                    stack: e.stack,
                },
