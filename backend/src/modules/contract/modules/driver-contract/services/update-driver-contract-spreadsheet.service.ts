...
@Injectable()
export class UpdateDriverContractSpreadsheetService extends SpreadsheetService<UpdateDriverContractDto> {
    constructor(
    ...
    async checkValidity(spreadsheetData: any[], timezone?: string): Promise<UploadSpreadsheetResult<UpdateDriverContractDto>> {
        const results: UploadSpreadsheetResult<UpdateDriverContractDto> = new UploadSpreadsheetResult<UpdateDriverContractDto>();
        await this.checkDtoValid(spreadsheetData, results, timezone);
        await this.checkDatabaseValid(results);
        return results;
    }

    async checkDtoValid(data: any[], results: UploadSpreadsheetResult<UpdateDriverContractDto>, timezone?: string): Promise<void> {
        this.logger.debug({ message: 'Checking Dto valid', detail: undefined, fn: this.checkDtoValid.name });
        // Check row by row
        for (const row of data) {
            const item = plainToClass(UpdateDriverContractDto, {
                ...row,
    ...
    async checkDatabaseValid(results: UploadSpreadsheetResult<UpdateDriverContractDto>): Promise<void> {
    ...
    async upload(dto: SpreadsheetUploadDto, file: MulterFile, socket: DistributedWebsocket, timezone?: string) {
        ...
        this.logger.log({ message: 'Uploading valid driver contracts', detail: verifyResults.valid.length, fn: this.upload.name });
        ...
        const jobs = from(verifyResults.valid).pipe(
            ...
            mergeMap((item) => {
                return from(this.updateDriverContractService.updateFromSpreadsheet(item)).pipe(
                    // If the job fails push to invalid verifyResults
                    catchError((e) => {
                        this.logger.warn({
                            message: 'Failed to upload driver contract',
                            detail: {
                                error: e.message,
                                stack: e.stack,
                            },
        ...
        this.logger.log({ message: 'Uploaded driver contracts', detail: results.valid.length, fn: this.upload.name });
    ...
    async generateValidDocument(data: any): Promise<number> {
        return super.generateValidDocument(data, 'valid-driver-contracts', 'driver-contract-uploads');
    }

    async generateInvalidDocument(data: any): Promise<number> {
        return super.generateInvalidDocument(data, 'invalid-driver-contracts', 'driver-contract-uploads');
    }
}
