...
/**
 * This is based on the map status function in the shohar crawler-api code
 */
const shoharFineStatusMapper = {
    1: 'פתוח',
    2: 'שולם',
    7: 'סגור',
};

export class ShoharRawInfringementMapper extends RawInfringementMapper {
    ...
    async getCreateInfringementDto(raw: RawInfringement): Promise<CreateInfringementDto> {
        ...
        const createInfringementData: CreateInfringementDto = {
            ...
            issuer: issuer.code,
            issuerStatus: shoharFineStatusMapper[fine.fine_status],
    ...
    private async getIssuer(externalIssuerCode: string): Promise<Issuer> {
        return this.issuersIntegration.getIssuerByExternalCodeAndType(externalIssuerCode, IssuerIntegrationType.Shohar);
    }

    private async getRawInfringementDto(raw: RawInfringement): Promise<ShoharRawInfringementDto> {
        const fine: ShoharRawInfringementDto = plainToClass(ShoharRawInfringementDto, raw.data);
