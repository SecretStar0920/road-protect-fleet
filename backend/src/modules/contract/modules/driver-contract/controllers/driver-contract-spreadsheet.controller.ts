...
import { CreateDriverContractSpreadsheetService } from '../services/create-driver-contract-spreadsheet.service';
...
@Controller('driver-contract/spreadsheet')
@UseGuards(UserAuthGuard)
export class DriverContractSpreadsheetController {
    constructor(
        private createDriverContractSpreadsheetService: CreateDriverContractSpreadsheetService,
        private updateDriverContractSpreadsheetService: UpdateDriverContractSpreadsheetService,
    ...
    async verify(@UploadedFile() file: Express.Multer.File, @Body('body') body: string): Promise<SpreadsheetUploadCompleteResponse> {
        ...
        if (parsedBody.method === 'create') {
            return await this.createDriverContractSpreadsheetService.verify(parsedBody, file);
        } else if (parsedBody.method === 'update') {
            return await this.updateDriverContractSpreadsheetService.verify(parsedBody, file);
        } else {
    ...
    async upload(@UploadedFile() file: Express.Multer.File, @Body('body') body: string, @UserSocket() socket: DistributedWebsocket) {
        ...
        if (parsedBody.method === 'create') {
            this.createDriverContractSpreadsheetService.upload(parsedBody, file, socket).catch((e) => {
                throw e;
            });
        } else if (parsedBody.method === 'update') {
            this.updateDriverContractSpreadsheetService.upload(parsedBody, file, socket).catch((e) => {
                throw e;
            });
        } else {
            throw new BadRequestException({ message: ERROR_CODES.E033_MethodNotSupportedORProvidedIncorrectly.message() });
