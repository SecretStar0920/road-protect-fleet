...
@Injectable()
export class CreateDriverContractSpreadsheetService extends SpreadsheetService<CreateDriverContractDto> {
    constructor(
    ...
    async checkValidity(spreadsheetData: any[]): Promise<UploadSpreadsheetResult<CreateDriverContractDto>> {
        const results: UploadSpreadsheetResult<CreateDriverContractDto> = new UploadSpreadsheetResult<CreateDriverContractDto>();
        await this.checkDtoValid(spreadsheetData, results);
        await this.checkDatabaseValid(results);
        return results;
    }

    async checkDtoValid(data: any[], results: UploadSpreadsheetResult<CreateDriverContractDto>): Promise<void> {
        this.logger.debug({ message: 'Checking Dto valid', detail: undefined, fn: this.checkDtoValid.name });
        // Check row by row
        for (const row of data) {
            const item = plainToClass(CreateDriverContractDto, row);
    ...
    async checkDatabaseValid(results: UploadSpreadsheetResult<CreateDriverContractDto>): Promise<void> {
    ...
    async upload(dto: SpreadsheetUploadDto, file: MulterFile, socket: DistributedWebsocket) {
        ...
        const jobs = from(verifyResults.valid).pipe(
            ...
            mergeMap(
                (item) => {
                    return from(this.createDriverContractService.createDriverContract(item)).pipe(
