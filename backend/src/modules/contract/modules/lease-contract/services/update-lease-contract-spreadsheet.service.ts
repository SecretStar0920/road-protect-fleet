...
import { UpdateLeaseContractDto } from '@modules/contract/modules/lease-contract/controllers/lease-contract.controller';
...
@Injectable()
export class UpdateLeaseContractSpreadsheetService extends SpreadsheetService<UpdateLeaseContractDto> {
    constructor(
    ...
    async checkValidity(spreadsheetData: any[], timezone?: string): Promise<UploadSpreadsheetResult<UpdateLeaseContractDto>> {
        const results: UploadSpreadsheetResult<UpdateLeaseContractDto> = new UploadSpreadsheetResult<UpdateLeaseContractDto>();
        await this.checkDtoValid(spreadsheetData, results, timezone);
        await this.checkDatabaseValid(results);
        return results;
    }

    async checkDtoValid(data: any[], results: UploadSpreadsheetResult<UpdateLeaseContractDto>, timezone?: string): Promise<void> {
        this.logger.debug({ message: 'Checking Dto valid', detail: undefined, fn: this.checkDtoValid.name });
        // Check row by row
        for (const row of data) {
            const item = plainToClass(UpdateLeaseContractDto, { ...row, timezone: timezone || row.timezone || Config.get.app.timezone });
    ...
    async checkDatabaseValid(results: UploadSpreadsheetResult<UpdateLeaseContractDto>): Promise<void> {
    ...
    async upload(dto: SpreadsheetUploadDto, file: MulterFile, socket: DistributedWebsocket, timezone?: string) {
        ...
        const jobs = from(verifyResults.valid).pipe(
            ...
            mergeMap((item) => {
                return from(this.updateLeaseContractService.updateFromSpreadsheet(item)).pipe(
