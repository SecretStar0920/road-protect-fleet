...
/**
 * This is based on the map status function in the kfarSaba crawler-api code
 */
const kfarSabaFineStatusMapper = {
    1: 'פתוח',
    2: 'שולם',
    7: 'סגור',
};

export class KfarSabaRawInfringementMapper extends RawInfringementMapper {
    ...
    async getCreateInfringementDto(raw: RawInfringement): Promise<CreateInfringementDto> {
        ...
        const createInfringementData: CreateInfringementDto = {
            ...
            issuer: issuer.code,
            issuerStatus: kfarSabaFineStatusMapper[fine.fine_status],
    ...
    private async getIssuer(externalIssuerCode: string): Promise<Issuer> {
        return this.issuersIntegration.getIssuerByExternalCodeAndType(externalIssuerCode, IssuerIntegrationType.KfarSaba);
    }

    private async getIssuerById(issuerId: number): Promise<Issuer> {
        return this.issuersIntegration.getIssuerByIssuerId(issuerId);
    }
    private async getRawInfringementDto(raw: RawInfringement): Promise<KfarSabaRawInfringementDto> {
        const fine: KfarSabaRawInfringementDto = plainToClass(KfarSabaRawInfringementDto, raw.data);
