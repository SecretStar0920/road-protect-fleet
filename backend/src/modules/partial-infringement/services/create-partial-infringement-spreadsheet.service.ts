...
@Injectable()
export class CreatePartialInfringementSpreadsheetService extends SpreadsheetService<PartialInfringementDetailsDto> {
    constructor(
    ...
    async checkValidity(spreadsheetData: any[], timezone?: string): Promise<UploadSpreadsheetResult<PartialInfringementDetailsDto>> {
        const results: UploadSpreadsheetResult<PartialInfringementDetailsDto> = new UploadSpreadsheetResult<
            PartialInfringementDetailsDto
        >();
    ...
    async checkDtoValid(data: any[], results: UploadSpreadsheetResult<PartialInfringementDetailsDto>, timezone?: string): Promise<void> {
        this.logger.debug({ message: 'Checking Dto valid', detail: undefined, fn: this.checkDtoValid.name });
        // Check row by row
        for (const row of data) {
            const item = plainToClass(PartialInfringementDetailsDto, {
                ...row,
    ...
    async checkDatabaseValid(results: UploadSpreadsheetResult<PartialInfringementDetailsDto>): Promise<void> {
    ...
    async upload(dto: SpreadsheetUploadDto, file: MulterFile, socket: DistributedWebsocket, timezone?: string) {
        ...
        this.logger.log({ message: 'Uploading valid partial infringements', detail: verifyResults.valid.length, fn: this.upload.name });
        ...
        const jobs = from(verifyResults.valid).pipe(
            ...
            mergeMap((item) => {
                return from(this.createPartialInfringementService.createPartialInfringementFromSpreadSheet(item)).pipe(
                    // If the job fails push to invalid verifyResults
                    catchError((e) => {
                        this.logger.warn({
                            message: 'Failed to upload partial infringements',
                            detail: {
                                error: e.message,
                                stack: e.stack,
                            },
        ...
        this.logger.log({ message: 'Uploaded partial infringements', detail: results.valid.length, fn: this.upload.name });
    ...
    async generateValidDocument(data: any): Promise<number> {
        return super.generateValidDocument(data, 'valid-partial-infringement', 'partial-infringement-uploads');
    }

    async generateInvalidDocument(data: any): Promise<number> {
        return super.generateInvalidDocument(data, 'invalid-partial-infringement', 'partial-infringement-uploads');
    }
}
