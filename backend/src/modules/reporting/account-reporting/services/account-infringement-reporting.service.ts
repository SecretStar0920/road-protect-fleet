...
@Injectable()
export class AccountInfringementReportingService {
    ...
    async getInfringementData(accountId: number): Promise<ReportingDataDto<MultiSeries>> {
        const data: ReportingDataDto<MultiSeries> = {
            data: [
                { name: 'Nomination Counts', series: [] },
                { name: 'Vehicle Infringement Counts', series: [] },
            ],
        };

        const nominated: Infringement[] = await Infringement.createQueryBuilder('infringement')
            .select(['infringement.infringementId', 'infringement.noticeNumber', 'infringement.status', 'infringement.amountDue'])
            .innerJoin('infringement.nomination', 'nomination')
            .innerJoin('nomination.account', 'account', 'account.accountId = :id', { id: accountId })
            .addSelect('nomination.status')
            .getMany();
        const nominatedFormatted: any = groupBy<Infringement>(nominated, (n) => n.nomination.status);
