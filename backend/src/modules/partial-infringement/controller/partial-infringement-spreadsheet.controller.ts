...
import { CreatePartialInfringementSpreadsheetService } from '../services/create-partial-infringement-spreadsheet.service';
...
@Controller('partial-infringement/spreadsheet')
@UseGuards(UserAuthGuard)
export class PartialInfringementSpreadsheetController {
    constructor(
        private createPartialInfringementSpreadsheetService: CreatePartialInfringementSpreadsheetService,
    ...
    async verify(@UploadedFile() file: Express.Multer.File, @Body('body') body: string): Promise<SpreadsheetUploadCompleteResponse> {
        ...
        if (parsedBody.method === 'create') {
            return await this.createPartialInfringementSpreadsheetService.verify(parsedBody, file);
        } else {
    ...
    async upload(
        ...
        if (parsedBody.method === 'create') {
            this.createPartialInfringementSpreadsheetService.upload(parsedBody, file, socket).catch((e) => {
                throw e;
            });
        } else {
            throw new BadRequestException({ message: ERROR_CODES.E033_MethodNotSupportedORProvidedIncorrectly.message() });
