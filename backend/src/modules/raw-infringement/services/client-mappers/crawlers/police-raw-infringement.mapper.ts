...
/**
 * This is based on the map status function in the police crawler-api code
 */
const policeFineStatusMapper = {
    1: 'פתוח',
    2: 'שולם',
    7: 'סגור',
};

export class PoliceRawInfringementMapper extends RawInfringementMapper {
    ...
    async getCreateInfringementDto(raw: RawInfringement): Promise<CreateInfringementDto> {
        ...
        const createInfringementData: CreateInfringementDto = {
            ...
            caseNumber: fine.fine_trx_msg,
            reason: fine.fine_comments,
            issuer: issuer.code,
            issuerStatus: policeFineStatusMapper[fine.fine_status],
    ...
    private async getAccount(fine: PoliceRawInfringementDto): Promise<Account> {
    ...
    private async getRawInfringementDto(raw: RawInfringement): Promise<PoliceRawInfringementDto> {
        const fine: PoliceRawInfringementDto = plainToClass(PoliceRawInfringementDto, raw.data);
    ...
    async getNominationDtoFromRawInfringement(raw: RawInfringement): Promise<NominationDto> {
        const fine: PoliceRawInfringementDto = await this.getRawInfringementDto(raw);
        if (!fine) {
            return {};
        }
