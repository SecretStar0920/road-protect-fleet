...
@Controller('account/spreadsheet')
@UseGuards(UserAuthGuard)
export class AccountSpreadsheetController {
    constructor(private accountSpreadsheetService: AccountSpreadsheetService, private antivirusService: AntivirusService) {}
    ...
    async verifyAccounts(
    ...
    ): Promise<SpreadsheetUploadCompleteResponse> {
        ...
        if (parsedBody.method === 'create') {
            return await this.accountSpreadsheetService.verify(parsedBody, file);
    ...
    async uploadAccounts(
        ...
        if (parsedBody.method === 'create') {
            this.accountSpreadsheetService.upload(parsedBody, file, socket).catch((e) => {
                throw e;
            });
        } else {
            throw new NotImplementedException({ message: ERROR_CODES.E033_MethodNotSupportedORProvidedIncorrectly.message() });
